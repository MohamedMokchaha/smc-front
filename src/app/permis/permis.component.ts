...
export class PermisComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private router: Router) {
    this.permisForm = this.fb.group({
      permisA1: ['', Validators.required],
      permisA2: ['', Validators.required],
      permisA: ['', Validators.required],
      permisB: ['', Validators.required],
  ...
  goBack() {
    this.router.navigate(['/vehicle-info']); // Redirection vers la page des informations du véhicule
  }
}
